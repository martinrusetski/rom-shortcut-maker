...
```
RomShortcutMaker/
├── RomShortcutMaker/          # Main application target
...
│   ├── RomShortcutMakerApp.swift  # App entry point
│   └── Info.plist                   # App configuration
├── RomShortcutMakerTests/     # Unit tests
...
Open `Package.swift` in Xcode and press ⌘R. SwiftPM is the canonical build system; the local `.xcodeproj` is a legacy, untracked wrapper and is not required.

## Testing

Run the unit tests headlessly with Swift Package Manager:
```bash
swift test
```
This builds the app target and runs the full suite without launching the GUI.
